...
fun validateCreditCard(creditCardNumber: String): Boolean {
    val cardIntArray = IntArray(creditCardNumber.length)

    var forwardIndex = 0
    var reverseIndex = (creditCardNumber.length) - 2
    while (reverseIndex >= 0) {
        val c: Char = creditCardNumber[reverseIndex]
        cardIntArray[forwardIndex] = ("" + c).toInt()
        forwardIndex = forwardIndex + 1
        reverseIndex = reverseIndex - 1
    }
    cardIntArray[forwardIndex] = (creditCardNumber[creditCardNumber.length - 1] + "").toInt()

    var i = 0
    while (i <= cardIntArray.size - 2) {
    ...
    var sum = 0
    for (i in 0 until cardIntArray.size) {
        sum = sum + cardIntArray[i]
    }
